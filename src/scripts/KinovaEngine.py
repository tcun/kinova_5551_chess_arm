...
class PickAndPlace(object):
    ...
    def __init__(self):
        ...
        try:
            self.is_gripper_present = rospy.get_param(
                "/my_gen3_lite/is_gripper_present", False)
            if self.is_gripper_present:
                print('gripper present!')
                gripper_joint_names = rospy.get_param(
                    "/my_gen3_lite/gripper_joint_names", [])
                self.gripper_joint_name = gripper_joint_names[0]
            else:
                print('gripper not present!')
                gripper_joint_name = ""
            self.degrees_of_freedom = rospy.get_param(
                "/my_gen3_lite/degrees_of_freedom", 7)

            # Create the MoveItInterface necessary objects
            arm_group_name = "arm"
            self.robot = moveit_commander.RobotCommander(
                "/my_gen3_lite/robot_description")
            self.scene = moveit_commander.PlanningSceneInterface(
                ns="/my_gen3_lite/")
            self.arm_group = moveit_commander.MoveGroupCommander(
                arm_group_name, robot_description="/my_gen3_lite/robot_description", ns="/my_gen3_lite/")
            ...
            if self.is_gripper_present:
                ...
                self.gripper_group = moveit_commander.MoveGroupCommander(
                    gripper_group_name, robot_description="/my_gen3_lite/robot_description", ns="/my_gen3_lite/")
    ...
    def reach_gripper_position(self, relative_position):
        ...
        try:
            val = gripper_joint.move(relative_position * (gripper_max_absolute_pos -
                                     gripper_min_absolute_pos) + gripper_min_absolute_pos, True)
            return val
        except:
            return False
